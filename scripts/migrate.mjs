...
try{await new PostgresAriveRepository(pool).initialize();await apply('010_historical_funding_corrections','../db/010_historical_funding_corrections.sql');await apply('016_private_my_work','../db/016_private_my_work.sql');await apply('017_operational_snapshot_merge','../db/017_operational_snapshot_merge.sql');await apply('018_operational_snapshot_recovery_audit','../db/018_operational_snapshot_recovery_audit.sql');await apply('019_conditions_follow_up_workflow','../db/019_conditions_follow_up_workflow.sql');console.log('HFN database schema initialized.');}finally{await pool.end();}

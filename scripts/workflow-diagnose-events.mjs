...
try{
  ...
  const row=loan.rows[0],events=await pool.query(`select id,"source",external_id "sourceEventId",received_at "receivedAt",processing_status "processingStatus",payload->>'currentLoanStatus' "currentLoanStatus",payload->>'triggerSource' "triggerSource",payload->>'suppliedEventId' "suppliedEventId",payload->'incomingFieldPresence' "incomingFieldPresence",payload->'topLevelKeys' "topLevelKeys",payload->'payloadStructure' "payloadStructure",payload->'trackerContext' "trackerContext",payload->'milestoneDates' "milestoneDates" from inbound_events where source='zapier-arive' and payload->>'ariveSystemGuid'=$1 order by received_at asc,id asc`,[row.ariveSystemGuid]);

...
async function withServer(employee,run){const store={adminCommandCenter:async()=>({summary:{activeLoans:1,pastDueLoans:0,ctcAtRisk:0,ctcCritical:0,managementEscalations:0},processors:[],assistants:[],escalations:[]}),health:async()=>({connectionStatus:'arive-connected',lastEventReceived:'2026-09-15T20:00:00Z',eventsReceivedToday:2,eventsProcessed:2,failedEvents:0,duplicateEventsIgnored:0,loansSynced:1,recent:[{receivedAt:'2026-09-15T20:00:00Z',sourceEventId:'evt-safe',ariveDisplayLoanId:'1001',eventType:'LOAN_SETUP',status:'processed',auditNote:'Processed'}]})},auth={session:async()=>employee},server=createHfnServer({store,auth,environment:'production'});await new Promise(resolve=>server.listen(0,resolve));try{await run(`http://127.0.0.1:${server.address().port}`);}finally{await new Promise(resolve=>server.close(resolve));}}
test('Admin Command Center API and page remain authenticated Admin-only',async()=>{await withServer({email:'admin@hfn.test',role:'admin'},async base=>{const response=await fetch(`${base}/api/admin/command-center`),body=await response.json();assert.equal(response.status,200);assert.equal(body.integrationHealth.connectionStatus,'arive-connected');assert.equal(JSON.stringify(body.integrationHealth).includes('borrower'),false);assert.equal((await fetch(`${base}/admin`)).status,200);});await withServer({email:'processor@hfn.test',role:'processor'},async base=>{assert.equal((await fetch(`${base}/api/admin/command-center`)).status,403);assert.equal((await fetch(`${base}/admin`)).status,403);});});

...
test('Processor review outcomes and lightweight workflow aging remain visible in My Work',async()=>{const [script,agingStyles]=await Promise.all([readFile(new URL('../public/my-work.js',import.meta.url),'utf8'),readFile(new URL('../public/my-work-aging.css',import.meta.url),'utf8')]);for(const label of ['Reviewed — No Action Needed','Add to Next UW Submission','Issue Found / Follow-Up Required'])assert.match(script,new RegExp(label));assert.match(script,/review-issue-dialog/);assert.match(script,/Stalled/);assert.match(script,/workflowAgingForTask/);assert.match(script,/stalledLoanCount/);assert.match(script,/my-work-aging\.css/);assert.match(agingStyles,/\.aging-overdue/);assert.match(agingStyles,/\.aging-waiting_external/);});
...
test('private My Work renders a safe underwriting submission round',async()=>{const script=await readFile(new URL('../public/my-work.js',import.meta.url),'utf8');assert.match(script,/UNDERWRITING SUBMISSION/);assert.match(script,/Ready for submission/);assert.match(script,/Relevant outstanding work/);assert.match(script,/ARIVE CONFIRMED/);assert.doesNotMatch(script,/rawPayload|documentContent/);});

...
test('dashboard count query qualifies joined state and other joined columns',async()=>{const source=await readFile(new URL('../src/storage/postgres-arive.js',import.meta.url),'utf8');assert.match(source,/m\.state='breached'/);assert.match(source,/e\.event_type='LOAN_FUNDED'/);assert.match(source,/t\.completed_at is null/);assert.doesNotMatch(source,/and state='breached'/);});

...
test('authorized bearer webhook is accepted',async()=>withServer(async base=>{const response=await send(base,{Authorization:'Bearer unit-secret'});assert.equal(response.status,200);assert.equal((await response.json()).tasksCreated,6);}));
test('duplicate authorized delivery is idempotent',async()=>withServer(async(base,store)=>{await send(base,{'X-HFN-Webhook-Secret':'unit-secret'});const response=await send(base,{'X-HFN-Webhook-Secret':'unit-secret'});assert.equal((await response.json()).outcome,'duplicate');assert.equal(store.integration.tasks.length,6);}));

...
test('settlement statement follow-up remains inside the existing single-loan Kanban grouping',()=>{const board=buildLoanKanban([task('order','17512793','action_required',{taskType:'order_settlement_statement'}),task('follow','17512793','waiting',{taskType:'settlement_statement_follow_up'})],{now});assert.equal(board.loans.length,1);assert.equal(board.loans[0].tasks.length,2);});

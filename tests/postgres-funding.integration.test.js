...
test('PostgreSQL dashboard Funded MTD reuses the merged funding aggregate',{skip:!url},async t=>{
  ...
  try{
    await client.query(`create schema ${schema};set search_path to ${schema}`);
    await client.query(`create table loans(id uuid primary key,arive_display_loan_id text,loan_amount numeric,processing_eligible_at timestamptz,current_stage text,kpi_tracking_started_at timestamptz);create table loan_stage_events(id uuid primary key,loan_id uuid,event_type text,occurred_at timestamptz,received_at timestamptz,metadata jsonb);create table historical_funding_corrections(display_loan_id text primary key,funded_date date,loan_amount numeric);create table monthly_funding_goals(year smallint,month smallint,funded_loan_goal integer,funded_volume_goal numeric);create table assistant_tasks(loan_id uuid,task_type text,kpi_eligible boolean,applicable_at timestamptz,due_at timestamptz,completed_at timestamptz);create table sla_rules(id uuid primary key,metric_key text,owner_role text);create table sla_measurements(loan_id uuid,rule_id uuid,state text,due_at timestamptz,evaluated_at timestamptz)`);
    const add=async(display,amount,at)=>{const id=randomUUID();await client.query('insert into loans values($1,$2,$3,$4,$5,$6)',[id,display,amount,'2026-01-01','LOAN_FUNDED','2026-01-01']);await client.query('insert into loan_stage_events values($1,$2,$3,$4,$5,$6)',[randomUUID(),id,'LOAN_FUNDED',at,at,{}]);};

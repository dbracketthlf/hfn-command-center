...
test('request-shape observability captures only allowlisted top-level operational presence before normalization',()=>{const payload={ariveLoanId:'safe-guid',titleStatus:'RECEIVED',titleReceivedDate:'2026-09-11',hoiStatus:null,data:{borrowerEmail:'not-retained@example.test'},borrowerFirstName:'Not Retained',unknownBorrowerField:'Not Retained',triggerSource:'loan-trackers-updated',zapierEventId:'zap-run-123'};
  ...
  assert.deepEqual(metadata.payloadStructure,{hasData:true,hasFields:false,hasLoan:false,hasTitleTrackerObject:false,hasAppraisalTrackerObject:false,hasHoiTrackerObject:false});
...
test('event history marks pre-observability rows unavailable without guessing their request shape',()=>{const [event]=safeInboundEventHistory([{id:'historic',receivedAt:'2026-09-13T23:42:49.402Z',source:'zapier-arive',sourceEventId:'safe-event-id',trackerContext:{},milestoneDates:{}}]);assert.equal(event.incomingFieldPresence,'UNAVAILABLE_FOR_HISTORICAL_EVENT');assert.equal(event.topLevelKeys,'UNAVAILABLE_FOR_HISTORICAL_EVENT');assert.equal(event.payloadStructure,'UNAVAILABLE_FOR_HISTORICAL_EVENT');assert.equal(event.recognizedTrackerFieldLocations,'UNAVAILABLE_FOR_HISTORICAL_EVENT');assert.equal(event.sourceEventId,'safe-event-id');});

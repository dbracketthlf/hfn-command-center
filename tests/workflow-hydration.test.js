...
test('hydration creates separate waiting follow-up tasks for proven ordered work',()=>{const plan=planHydratedWorkflow({stage:'UNDERWRITING_SUBMITTED',purpose:'purchase',milestoneDates:{appraisalOrderedDate:'2026-09-10T18:00:00Z',titleOrderedDate:'2026-09-10T18:00:00Z',hoiOrderedDate:'2026-09-10T18:00:00Z'},...owners});assert.deepEqual(plan.tasks.map(task=>task.type),['appraisal_follow_up','title_follow_up','insurance_follow_up']);assert.ok(plan.tasks.every(task=>task.state==='waiting'&&task.kpiEligible===false));});

...
export function createHfnServer({ store=createAriveStore(), environment=process.env.NODE_ENV ?? 'development', webhookSecret, auth=null }={}) {
  return createServer(async (req,res) => {
    ...
    try {
      ...
      if(req.method==='GET'&&url.pathname==='/api/my-work'){if(!employee)return json(res,401,{ok:false,error:'Sign in required'});const queue=store.myWorkQueue??store.processorWorkQueue;if(!queue)return json(res,503,{ok:false,error:'Work queue unavailable'});if(!['processor','processor_assistant','admin'].includes(employee.role))return json(res,403,{ok:false,error:'My Work is not available for this role'});return json(res,200,await queue.call(store,{employee,ownerEmail:employee.role==='admin'?url.searchParams.get('ownerEmail'):null}));}

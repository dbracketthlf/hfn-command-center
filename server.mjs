...
export function createHfnServer({ store=createAriveStore(), environment=process.env.NODE_ENV ?? 'development', webhookSecret, auth=null }={}) {
  return createServer(async (req,res) => {
    ...
    try {
      ...
      if(req.method==='POST'&&url.pathname.startsWith('/api/my-work/tasks/')&&url.pathname.endsWith('/actions')){if(!employee)return json(res,401,{ok:false,error:'Sign in required'});if(!store.mutateWorkflowTask)return json(res,503,{ok:false,error:'Task actions unavailable'});const taskId=decodeURIComponent(url.pathname.slice('/api/my-work/tasks/'.length,-'/actions'.length)),body=await readJson(req);return json(res,200,await store.mutateWorkflowTask({employee,taskId,action:body.action,blockedCategory:body.blockedCategory??null,note:body.note??null,checklistItemId:body.checklistItemId??null,checklistState:body.checklistState??null}));}
